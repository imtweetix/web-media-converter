...
export function FileItem({
  ...
  const handleVideoSettingChange = (updates: Partial<VideoSettings>) => {
    const currentSettings = file.videoSettings || { resolution: 'default', crf: 28, fps: 'default', audioEnabled: true };
    onUpdateVideoSettings(file.id, {
      ...currentSettings,
      ...updates,
      enabled: true
        ...
        <div className="flex space-x-4">
          ...
          {(file.convertedPreview || file.status === 'converted') && (
            <div className="flex flex-col items-center">
              ...
              <span className="text-xs text-gray-500 mt-1">{file.isVideo ? 'WebM' : 'WebP'}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center space-x-4 text-sm text-gray-600 mb-2">
            ...
            {file.convertedSize && (
              <>
                <span>→</span>
                <span>{file.isVideo ? 'WebM' : 'WebP'}: {formatFileSize(file.convertedSize)}</span>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => handleVideoSettingChange({
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => handleVideoSettingChange({ crf: Number(e.target.value) })}
                      className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer focus:outline-none focus:ring-1 focus:ring-indigo-500"
                    />
                    <div className="flex justify-between text-xs text-gray-500 mt-1">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={e => handleVideoSettingChange({ audioEnabled: e.target.checked })}
                      className="h-3 w-3 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                    />
                    <label
